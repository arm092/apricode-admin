...
<div class="flex shadow-sm">
    @if ($this->isFilterable())
        <select wire:model="filter" class="text-sm flex-shrink-0 sm:text-base focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 border-secondary-100 {{ $this->isSearchable() ? 'rounded-l' : 'rounded' }}">
    ...
    @if ($this->isSearchable())
        <div class="relative flex-grow">
            <input
                ...
                class="text-sm sm:text-base pl-10 block w-full placeholder-gray-400 focus:placeholder-gray-500 placeholder-opacity-100 focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 border-secondary-100 {{ $this->isFilterable() ? 'rounded-r border-l-0' : 'rounded' }}"
            />
